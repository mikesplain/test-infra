...
func validateJobBase(v JobBase, jobType prowapi.ProwJobType, podNamespace string) error {
	...
	if err := ValidatePipelineRunSpecV2(jobType, v.ExtraRefs, v.PipelineRunSpecV2); err != nil {
		return err
	}
...
func validateAgent(v JobBase, podNamespace string) error {
	...
	case v.PipelineRunSpecV2 != nil && agent != p:
		return fmt.Errorf("job pipeline_run_specV2 require agent: %s (found %q)", p, agent)
	case agent == p && v.PipelineRunSpecV2 == nil:
		return fmt.Errorf("agent: %s jobs require a pipeline_run_spec", p)
...
func ValidatePipelineRunSpecV2(jobType prowapi.ProwJobType, extraRefs []prowapi.Refs, spec *pipelinev1beta1.PipelineRunSpec) error {
	if spec == nil {
		return nil
	}
	// Validate that that the refs match what is requested by the job.
